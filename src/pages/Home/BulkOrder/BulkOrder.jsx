...
function BulkOrder() {
   ...
   const CallApiGetPreOrderCampaign = async (PageIndex, PageSize, type) => {
      const response = await ApiGetPreOrderCampaign({ PageIndex, PageSize, type })
      setListBulkOrder(response)
   }

   useEffect(() => {
      CallApiGetPreOrderCampaign(1, 10, 1)
   }, [])
         ...
         <div className="mb-[25px] relative bg-[#707070] rounded-[17px] pb-[40px] ">
            <div className="px-[50px] w-full ">
               ...
               <div className='relative '>
                  ...
                  {/* Nút trái */}
                  <div ref={navigationLeftRef} className='bg-none w-auto h-auto left-[-35px] right-auto absolute top-[50%] mt-[-22px] cursor-pointer leading-[1.6]'>
                     <img className='max-w-full h-auto align-middle '
                        src={LeftButton} alt="Nút bên trái" />
                  </div>
                  {/* Nút phải */}
                  <div ref={navigationRighttRef} className='bg-none w-auto h-auto right-[-35px] left-auto absolute top-[50%] mt-[-22px] cursor-pointer leading-[1.6]'>
                     <img className='max-w-full h-auto align-middle '
