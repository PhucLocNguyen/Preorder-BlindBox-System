...
function ApprovalCampaign() {
  ...
  const { data, loading, pagination } = useFetchDataPagination(fetchPreorderCampaign, [
    pageSize,
    pageIndex,
  ]);

  // Lọc dữ liệu theo tên chiến dịch dựa trên input search
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      sorter: (a, b) =>
        (a.blindBox?.name || "").localeCompare(b.blindBox?.name || ""),
      render: (_, record) => (
    ...
    },
    {
      ...
      align: "center",
      render: (_, record) => (
        <div className="text-center">{record.type || "Unknown"}</div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle" className="justify-between">
          <Button type="primary">
            <Link to={`/admin/preordercampaignApproval/confirm/${record.slug}`}>
              Xem chi tiết
            </Link>
          </Button>
        </Space>
      ...
      <div className="w-full mx-auto mt-5 p-5 bg-white shadow-lg rounded-lg">
        <div className="flex justify-between">
          <h2 className="text-2xl font-bold mb-4">
            Các chiến dịch gom đơn hàng đang chờ duyệt
          </h2>
        </div>
        ...
        {warning && (
          <div className="w-full text-right text-red-500 mt-2">{warning}</div>
        )}
